...
let adminPassword = "abcd"; // 관리자 비밀번호
...
// 게임 종료 함수 (자금 부족 시 호출)
const endGameDueToInsufficientFunds = () => {
    $('body').append(`
        <div id="end-message">
            <p>💸 자금이 부족하여 게임이 종료되었습니다! 💸</p>
            <p>다음에 다시 도전해주세요!</p>
        </div>
    `);
...
const disableAllControls = () => {
    ...
    $('#redeal').prop('disabled', true);
    $('#place-bet').prop('disabled', true);
    $('#fold').prop('disabled', true);
    $('#new-game').prop('disabled', true);
    $('#bet-slider').prop('disabled', true);
    $('#close-bet').prop('disabled', true);
    $('.high-low').hide(); // 숨김 처리
    $('#win-amount').hide(); // 숨김 처리
};
$('#redeal').click(() => {
    $('#redeal').hide();
    $('#place-bet').hide();
